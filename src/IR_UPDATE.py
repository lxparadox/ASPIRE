...
from PyQt5.QtWidgets import (QApplication, QWidget, QLabel, QLineEdit, QTextEdit, QPushButton,QVBoxLayout, QHBoxLayout,
                             QGroupBox, QComboBox, QDateEdit, QMessageBox, QButtonGroup, QRadioButton, QProgressDialog)
...
class ASPIRE_IR_update(QWidget):
    fetch_completed = pyqtSignal()  # 新增信号

    def __init__(self):
        super().__init__()
        self.project_root = Path(os.path.abspath(__file__)).parent.parent
        self.latest_period_file = self.project_root/"data"/"latest_period_IR.json"  # 用于保存最新的时间段
    ...
    def initUI(self):
        ...
        self.setWindowIcon(QIcon(self.logo_pixmap))  # 这个可以保留或者移除
        self.setStyleSheet("""
            QWidget {
    ...
    def create_title(self):
        title_label = QLabel("IR-related Gene Knowledge Graph Update")
    ...
    def start_fetching_literature(self):
        ...
        if not self.output_directory:
            ...
            output_dir_name = f'IR_result_{timestamp}'
    ...
    def calculate_and_run_tasks(self, start_date_dt, end_date_dt, output_file, output_file_prefix):
        ...
        terms = [
            'Ionizing Radiation', 'Alpha Particle', 'Alpha Ray', 'Alpha Radiation',
            'Beta Particle', 'Beta Ray', 'Beta Radiation', 'Gamma Wave',
            'Gamma Ray', 'Gamma Radiation', 'X Ray', 'Roentgen Ray', 'Grenz Ray'
        ]
    ...
    def start_generating_entities(self):
        self.log_output("Start to generate entities")

        # 获取用户输入
        base_filename = 'results_all_gene'  # 这可以是前端传入的参数
        biomedical_entity_terms = [
            'Ionizing Radiation', 'Ionizing Radiations', 'Ionizing-Radiation', 'Ionizing-Radiations',
            'Ionising Radiation', 'Ionising Radiations', 'Ionising-Radiation', 'Ionising-Radiations',
            'Alpha Particle', 'Alpha Particles', 'Alpha-Particle', 'Alpha-Particles', 'Alpha Ray', 'Alpha Rays',
            'Alpha-Ray', 'Alpha-Rays', 'Alpha Radiation', 'Alpha Radiations', 'Alpha-Radiation', 'Alpha-Radiations',
            'Beta Particle', 'Beta Particles', 'Beta-Particle', 'Beta-Particles', 'Beta Ray', 'Beta Rays', 'Beta-Ray',
            'Beta-Rays', 'Beta Radiation', 'Beta Radiations', 'Beta-Radiation', 'Beta Radiations',
            'Gamma Wave', 'Gamma Waves', 'Gamma-Wave', 'Gamma-Waves', 'Gamma Ray', 'Gamma Rays', 'Gamma-Ray',
            'Gamma-Rays', 'Gamma Radiation', 'Gamma Radiations', 'Gamma-Radiation', 'Gamma-Radiations',
            'X Ray', 'X Rays', 'X-Ray', 'X-Rays', 'Roentgen Ray', 'Roentgen Rays', 'Roentgen-Ray', 'Roentgen-Rays',
            'Grenz Ray', 'Grenz Rays', 'Grenz-Ray', 'Grenz-Rays'
        ]
        ...
        try:
            ...
            stats_biomedical = self.count_biomedical_results(biomedical_file, "all retrieved genes")
            stats_filtered = self.count_biomedical_results(filtered_file, "the unrecorded genes")

            combined_stats = f"{stats_filtered}\n\n{stats_biomedical}"
    ...
    def generate_filtered_gene_file(self):
        ...
        try:
            ...
            csv_file_path = self.data/'IR_all_data.csv'  # 筛选条件文件
        ...
        if checked:
            ...
            if hasattr(self, 'cached_filtered_results') and self.cached_filtered_results:
                ...
                try:
                    ...
                    csv_file_path = os.path.join(self.data/'IR_all_data.csv')
    ...
    def start_model_process(self):
        """根据选择的模型开始不同的处理过程"""
        model = self.model_selector.currentText()
        IR_prompt = ("**Role Setting**\n"
                     "You will play the role of a bioinformatics analyst named 'BioAnalyser,' responsible for evaluating each sentence I provide to determine whether a gene or protein is associated with ionizing radiation in terms of biological effects, and whether the sentence is a conclusive statement explaining this association. Each sentence contains the following columns: Index (first column), Gene or Protein Entity (fourth column), and Radiation Entity (seventh column).\n"
                     "**Task Description**\n"
                     "You need to determine whether the sentences provided by the user explicitly indicate a direct or indirect association between a gene or protein and ionizing radiation in terms of biological effects, and judge whether the sentence is a conclusive statement explaining this association. Please make determinations based on the following rules to identify the relationship between these two entities in the sentence.\n"
                     "**Chain of Thought Process**\n"
                     "For each sentence, please follow the steps below for evaluation:\n"
                     "1. **Entity Recognition**:\n"
                     "   - Extract the Index information from the first column.\n"
                     "   - Extract the Gene or Protein Entity from the fourth column.\n"
                     "   - Extract the Radiation Entity from the seventh column.\n"
                     "2. **Rule 1 Evaluation: Check Radiation Exposure**:\n"
                     "   - Determine whether ionizing radiation directly affects the gene or protein.\n"
                     "     - If there is radiation exposure, output <Yes>.\n"
                     "     - If the sentence involves ionizing radiation causing gene inactivation or gene independence, implying no biological effect, output <No>.\n"
                     "3. **Rule 2 Evaluation: Assess Impact**:\n"
                     "   - **Direct Impact**:\n"
                     "     - Check whether ionizing radiation leads to changes in gene expression levels or activity.\n"
                     "     - Determine whether ionizing radiation alters the structure, function, or chemical modification of proteins.\n"
                     "     - Evaluate whether the sentence describes ionizing radiation causing DNA damage or involving repair mechanisms.\n"
                     "     - If any direct impact is identified, output <Yes>.\n"
                     "   - **Indirect Impact**:\n"
                     "     - Identify whether ionizing radiation activates cellular stress responses or signaling pathways.\n"
                     "     - Look for mutations induced by ionizing radiation that affect gene or protein function.\n"
                     "     - Evaluate whether ionizing radiation induces apoptosis or necrosis involving specific genes or proteins.\n"
                     "     - If any indirect impact is identified, output <Yes>.\n"
                     "   - **Unrelated Impact**:\n"
                     "     - If the effects described in the sentence do not lead to direct or indirect changes in gene expression or protein activity, or are vaguely described, output <No>.\n"
                     "4. **Rule 3 Evaluation: Conclusive Statement Check**:\n"
                     "   - **Explicit Result Statement**:\n"
                     "     - The sentence should explicitly state experimental results or research findings, using verbs like 'indicate,' 'prove,' 'show,' 'discover,' 'reveal,' etc.\n"
                     "     - If it includes uncertainty words like 'may,' 'possibly' but does not affect the core relationship, it is still considered a conclusive statement.\n"
                     "     - If the above conditions are met, output <Yes>.\n"
                     "   - **Non-Conclusive Elements**:\n"
                     "     - Sentences that describe research background, objectives, or methods without direct results, output <No>.\n"
                     "     - If the core relationship includes uncertainty expressions such as 'may,' 'possibly,' 'hypothesize,' treat it as a non-conclusive statement, output <No>.\n"
                     "5. **Final Determination**:\n"
                     "   - Only when Rules 1, 2, and 3 all evaluate to <Yes>, output 'T'; otherwise, output 'F'.\n"
                     "6. **Output Format**:\n"
                     "Compile your conclusions in JSON format as follows:\n"
                     "'```json[{\"Index\": , \"answer\": }]```\n'"
                     )

        if model == "ERNIE4":
            self.process_ernie(IR_prompt)
        elif model == "Kimi":
            self.process_kimi(IR_prompt)
        elif model == "Sparkai4":
            self.process_sparkai(IR_prompt)
        elif model == "GLM4":
            self.process_glm4(IR_prompt)
        else:
    ...
    def process_ernie(self, IR_prompt):
        ...
        original_IR_path = input_txt_path
        output_txt_path = os.path.join(self.output_directory, f'{project_name}_ernie4_output.txt')
        output_csv_path = os.path.join(self.output_directory, f'{project_name}_ernie4_output.csv')
        combined_csv_path = os.path.join(self.output_directory, f'{project_name}_final_ernie4_output.csv')
        ...
        try:
            ...
            ernie.process_files(output_txt_path, output_csv_path, original_IR_path)
    ...
    def process_kimi(self, IR_prompt):
        ...
        try:
            ...
            self.log_output("Starting to process files...")
            kimi.process_files(output_txt_path, output_csv_path, original_IR_path)
    ...
    def process_sparkai(self, IR_prompt):
    ...
    def process_glm4(self, IR_prompt):
    ...
    def start_knowledge_graph_process(self):
        ...
        try:
            ...
            # 将合并后的文件覆盖当前目录下的 IR_all_data.csv
            shutil.copy(merged_output_file, self.data/'IR_all_data.csv')
            self.log_output("The CSV file has been successfully merged and overwritten to IR_all_data.csv")
            ...
            try:
                ...
                message = (
                    ...
                    f" - The main file IR_all_data.csv has been updated.\n"
                    f"2. Statistics:\n"
        ...
        self.progress_dialog = QProgressDialog("Generating Ionizing Radiation-related Gene Knowledge Graph, please wait...", "Cancel", 0, 0, self)
        self.progress_dialog.setWindowTitle("Loading")
        self.progress_dialog.setWindowModality(Qt.WindowModal)  # 设置为模态对话框
        self.progress_dialog.setCancelButton(None)  # 禁用取消按钮
        self.progress_dialog.show()
        ...
        try:
            # 最终输出文件路径
            final_output_file = os.path.join(self.output_directory, 'IR_gene_info_show.csv')

            # 调用后端函数：筛选基因信息并生成输出
            filter_and_query_genes(merged_output_file, final_output_file)

            # 将最终文件覆盖当前目录下的 IR_gene_info_show.csv
            shutil.copy(final_output_file, self.data/'IR_gene_info_show.csv')
            #self.log_output("The genetic information processing has been completed, and the results have been saved and overwritten to IR_gene_info_Show.csv.")
        except Exception as e:
        ...
        try:
            ...
            input_file = self.data/'IR_gene_info_show.csv'  # 输入文件路径
            output_file = os.path.join(self.output_directory, 'IR_gene_pathway_results.csv')  # 输出文件路径
            ...
            # 将输出文件覆盖当前目录下的 IR_gene_pathway_results.csv
            shutil.copy(output_file, self.data/'IR_gene_pathway_results.csv')
            #self.log_output("KEGG analysis is completed, and the results have been saved and overwritten to IR_gene_pathway_results.csv。")
    ...
    def update_and_display(self):
        # 定义新的文件路径
        new_gene_info_path = self.data/'IR_gene_info_show.csv'
        new_gene_pathway_path = self.data/'IR_gene_pathway_results.csv'
        new_all_data_path = self.data/'IR_all_data.csv'

        #print("Passing the following paths to the backend:")
        print(f"IR-related Gene information saved to {new_gene_info_path}")
        print(f"IR-related Gene pathway saved to {new_gene_pathway_path}")
