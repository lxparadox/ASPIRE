...
def process_chunk(chunk, prompt_task_header, client, max_retries, global_sleep_time):
    ...
    while retries < max_retries:
        try:
            # Call new API to get response using GLM-4 model
            response = client.chat.completions.create(
                model="glm-4-air",  # 使用GLM-4模型
                messages=messages
            )
            ...
            if hasattr(response, 'choices') and len(response.choices) > 0:
                ...
                return completion_message.splitlines()  # Assuming we want to return each response line by line
            else:
...
def process_data(input_txt_path, output_txt_path, prompt_task_header, client, max_retries, global_sleep_time):
    # read the TXT file
    ...
    for chunk_index, chunk in enumerate(chunks):
        ...
        completion_message = process_chunk(chunk, prompt_task_header, client, max_retries, global_sleep_time)
        if completion_message:  # Check if completion_message is not None
            chunk_result.extend(completion_message)
        else:
            print(f"No completion message received for chunk {chunk_index}.")
...
def process_missing_rows_and_loop(original_txt_path, initial_generated_csv_path, combined_csv_path, prefix,
                                  prompt_task_header, client, max_retries, global_sleep_time):
    iteration = 1
    ...
    while True:
        try:
            ...
            process_data(missing_output_txt_path, temp_missing_output_txt_path, prompt_task_header, client,
