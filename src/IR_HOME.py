...
from IR_UPDATE import ASPIRE_IR_update
import csv
...
class ASPIRE_IR(QWidget):
    ...
    def initUI(self):
        ...
        # 设置窗口图标
        self.setWindowIcon(QIcon(self.logo_pixmap))  # 使用logo.png作为窗口图标
        ...
        return_button = QPushButton("Go back to the main interface")
        return_button.clicked.connect(self.return_to_main)
        top_layout.addWidget(return_button)
        main_layout.addLayout(top_layout)      # 将水平布局添加到主布局
    ...
    def create_title(self):
        title_label = QLabel("Ionizing Radiation-related Gene Knowledge Graph")
    ...
    def create_operation_group(self):
        ...
        operation_text.setWordWrap(True)  # 自动换行
    ...
    def get_knowledge_graph_statistics(self):
        ...
        """
        csv_file_path = self.project_root/"data"/"IR_all_data.csv"
        gene_info_path = self.project_root/"data"/"IR_gene_info_show.csv"
        json_file_path = self.project_root/"data"/"latest_period_IR.json"
        ...
        try:
            ...
            return (f"Included literature: {unique_pmids} papers\n"
                    ...
                    f"IR-related genes: {unique_gene_ids_T} genes")
    ...
    def update_and_display(self):
        # 定义新的文件路径
        new_gene_info_path = self.project_root/"data"/'IR_gene_info_show.csv'
        new_gene_pathway_path = self.project_root/"data"/'IR_gene_pathway_results.csv'
        new_all_data_path = self.project_root/"data"/'IR_all_data.csv'
    ...
    def create_update_group(self):
        ...
        update_info_label = QLabel("Do you need to update the Ionizing Radiation-related gene knowledge graph?")
