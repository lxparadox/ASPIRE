...
def process_chunk(chunk, prompt_task_header, chat_comp, max_retries, global_sleep_time):
    ...
    while retries < max_retries:
        try:
            # Call new API to get response
            resp = chat_comp.do(model="ERNIE-4.0-8K", messages=messages)

            # If the response contains a result, return the result
            if 'result' in resp:
                completion_message = resp['result']
                print(f"Received completion for chunk: {completion_message}")
                return completion_message
            else:
                print(f"Warning: No 'result' found in completion response for chunk")
                retries += 1
            ...
            error_message = str(e)
            print(f"An error occurred for chunk: {e}")
            retries += 1
            if retries < max_retries:
                time.sleep(2 ** retries)
            else:
...
def process_data(input_txt_path, output_txt_path, prompt_task_header, chat_comp, max_retries, global_sleep_time):
    # Read TXT file
    ...
    for chunk_index, chunk in enumerate(chunks):
        ...
        completion_message = process_chunk(chunk, prompt_task_header, chat_comp, max_retries, global_sleep_time)
...
def process_files(input_txt_path, output_csv_path, original_uv_path):
    # Read original UV data
    original_uv = pd.read_csv(original_uv_path, sep='\t')

    # Create an empty list to store extracted results
    results = []

    # Read the contents of input_txt_path
    with open(input_txt_path, 'r', encoding='utf-8') as infile:
        for line in infile:
            # Use regex to match JSON formatted string, capturing Index and answer
            ...
            for match in matches:
                ...
                # Check if index is in original_uv
    ...
    if results:
        ...
        # Write results to output_csv_path
...
def process_missing_rows_and_loop(original_txt_path, initial_generated_csv_path, combined_csv_path, prefix,
                                  prompt_task_header, chat_comp, max_retries, global_sleep_time):
    iteration = 1
    ...
    while True:
        try:
            ...
            # If there are no missing rows, directly output results and exit loop
            ...
            process_data(missing_output_txt_path, temp_missing_output_txt_path, prompt_task_header, chat_comp,
